...
hansen <-
  raster("Hansen_reclass_90.tiff")   #  A raster to get distances from, should ideally be binary 1s and 0s where we are looking for distances to 1s
e <- extent(raster)
...
buff_crop <- function(x, buff) {
  ...
  ymin <- x$lat - buff
  ymax <- x$lat + buff
  xmin[xmin < round(e@xmin)] <-
    round(e@xmin)    #Ensuring the extent of the crop does not go beyond the bounds of the original raster
  ...
  test_pts <-
    test_pts[test_pts[, 3] == 1, ]   ##Here we are looking for raster cells with the value of one but this could be changed to be more flexible
...
hansen_dist_fun <- function(x) {
  ...
  if (extract_out == 1) {
    ...
    while (is.null(dist_out)) {
      if (i > length(bs)) {
        dist_out <-
          paste0("Location is further than ",
                 max(bs),
                 " degrees from nearest forest!")
        bs_out <- NA
      } else {
        test_pts <-
...
all_dist <-
  do.call("rbind", fun_out)  #rbinding the results together
